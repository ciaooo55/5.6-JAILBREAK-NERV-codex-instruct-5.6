...
| 项目 | 说明 |
| --- | --- |
| 中文名称 | GPT-5.6 NERV 指令与安全测试框架 |
| GitHub 仓库名 | `5.6-JAILBREAK-NERV-codex-instruct-5.6` |
| 名称状态 | 保持当前名称，本轮没有改名 |
| 主要用途 | 研究 Codex 指令上下文、代理中转、MCP 工具编排和模型拒绝行为 |
| 组成 | 配置、桥接提示词、代理服务、验证脚本、MCP 工具和技能模块 |
| 使用边界 | 仅用于自有设备、隔离实验环境或获得明确授权的安全测试 |

> 该项目包含模型限制规避、代理转发和安全测试相关内容，不是 OpenAI 官方项目。使用者应自行确认目标授权、数据安全和所在地法律要求，不要把真实凭据、生产系统或第三方目标带入实验。
